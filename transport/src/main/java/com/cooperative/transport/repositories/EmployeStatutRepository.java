...
public interface EmployeStatutRepository extends JpaRepository<EmployeStatut, Integer> {
    @Query(value = "SELECT es.id_statut FROM employe_statut es WHERE es.id_employe =:employeId ORDER BY es.dateModification DESC ,es.id desc LIMIT 1", nativeQuery = true)
